...
| `/opt/vpn` | Live VPN/media egress runtime and secret root | [[systems/prometheus/opt/stacks/media/vpn/gluetun/gluetun]] | Persistent runtime / Service config | Back up sanitized config only; secrets recovery needs validation | Compose source is `/opt/stacks/media/vpn/compose.yml`; `/opt/vpn/docker-compose.yml` is a legacy symlink; do not commit VPN secrets |
